...
/**
 * [DataSearchValidator][com.weedow.spring.data.search.validation.DataSearchValidator] implementation to check if the field expression value is less or equals to the specified [maxValue].
 */
class MaxValidator(
    ...
    override fun doValidate(value: Number, fieldExpression: FieldExpression, errors: DataSearchErrors) {
        if (compare(value, maxValue) > 0) {
            errors.reject("max", "Invalid number value for expression ''{0}''. Must less or equals to ''{1}''.", ExpressionUtils.format(fieldExpression), maxValue)
        }
    }
