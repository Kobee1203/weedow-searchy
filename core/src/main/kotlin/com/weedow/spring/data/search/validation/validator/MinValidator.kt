...
/**
 * [DataSearchValidator][com.weedow.spring.data.search.validation.DataSearchValidator] implementation to check if the field expression value is greater or equals to the specified [minValue].
 */
class MinValidator(
    ...
    override fun doValidate(value: Number, fieldExpression: FieldExpression, errors: DataSearchErrors) {
        if (compare(value, minValue) < 0) {
            errors.reject("min", "Invalid number value for expression ''{0}''. Must greater or equals to ''{1}''.", ExpressionUtils.format(fieldExpression), minValue)
        }
    }
