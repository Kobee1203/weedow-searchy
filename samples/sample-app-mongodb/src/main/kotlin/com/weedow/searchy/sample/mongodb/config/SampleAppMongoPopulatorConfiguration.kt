...
class SampleAppMongoPopulatorConfiguration {
    ...
    @Configuration
    @ConditionalOnProperty(value=["weedow.mongodb.sample.data.mode"], havingValue = "RESOURCE", matchIfMissing = false)
    internal class ResourcePopulatorConfiguration {

        @Value("#{'\${weedow.mongodb.sample.data:classpath:data.json}'.split(',')}")
        private lateinit var data: Array<Resource>
        ...
        @Bean
        fun taskKeyDeserializer(): Module {
    ...
    @Configuration
    @ConditionalOnProperty(value=["weedow.mongodb.sample.data.mode"], havingValue = "PROGRAMMATIC", matchIfMissing = true)
    internal class ProgrammaticPopulatorConfiguration {
        ...
        fun handleContextRefreshEvent(cre: ContextRefreshedEvent) {
            ...
            val person3 = personRepository.insert(
                Person(
                    "Bob",
                    "Nullos"
                )
