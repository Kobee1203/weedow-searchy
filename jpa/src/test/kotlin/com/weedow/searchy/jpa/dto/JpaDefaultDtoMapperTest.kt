package com.weedow.searchy.jpa.dto
...
internal class JpaDefaultDtoMapperTest {

    /**
     * Test to cover equals and hashcode method
     */
    @Test
    fun jpa_default_dto_mapper_equality() {
        val entityInitializer = mock<Initializer<Any>>()
        val dtoMapper1 = JpaDefaultDtoMapper<Person>(entityInitializer)
        val dtoMapper2 = JpaDefaultDtoMapper<Person>(entityInitializer)
        val dtoMapper3 = JpaDefaultDtoMapper<Person>()

        assertThat(dtoMapper1).isEqualTo(dtoMapper2)
        assertThat(dtoMapper1).isNotEqualTo(dtoMapper3)

        assertThat(dtoMapper1).hasSameHashCodeAs(dtoMapper2)
        assertThat(dtoMapper1.hashCode()).isNotEqualTo(dtoMapper3.hashCode())
    }

    @Test
    fun map_entity_to_dto() {
        val dtoMapper = JpaDefaultDtoMapper<Person>()
    ...
    fun map_entity_to_dto_with_skipped_property() {
        ...
        val dtoMapper = JpaDefaultDtoMapper<MyEntityWithSkippedProperty>(EntityInitializer(propertyInitializer, mapInitializer, collectionInitializer))
